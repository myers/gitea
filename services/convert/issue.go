...
func ToIssue(ctx context.Context, doer *user_model.User, issue *issues_model.Issue, opts ...ToIssueOptions) *api.Issue {
	return toIssue(ctx, doer, issue, cache.NewEphemeralCache(), WebAssetDownloadURL, mergeOpts(opts))
}
...
func ToAPIIssue(ctx context.Context, doer *user_model.User, issue *issues_model.Issue, opts ...ToIssueOptions) *api.Issue {
	return toIssue(ctx, doer, issue, cache.NewEphemeralCache(), APIAssetDownloadURL, mergeOpts(opts))
}
...
func toIssue(ctx context.Context, doer *user_model.User, issue *issues_model.Issue, permCache *cache.EphemeralCache, getDownloadURL func(repo *repo_model.Repository, attach *repo_model.Attachment) string, opts ToIssueOptions) *api.Issue {
...
func ToAPIIssueList(ctx context.Context, doer *user_model.User, il issues_model.IssueList, opts ...ToIssueOptions) []*api.Issue {
	...
	for i := range il {
		result[i] = toIssue(ctx, doer, il[i], permCache, APIAssetDownloadURL, o)
	}
	return result
}
